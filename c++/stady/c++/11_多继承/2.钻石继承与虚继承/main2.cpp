...
// 增加 virtual  关键字
class Bfor_A : virtual public Base_A
{
private:
    int age;
...
// 增加 virtual  关键字   让继承关系成为：虚继承
class Cfor_A : virtual public Base_A
{
private:
    int height;
...
public:
    ...
    void printf(void)
    {
        cout << "that height is:" << height << endl;
    }
};
// 派生类
class derivative : public Cfor_A, public Bfor_A
...
public:
    // 这里初始化列表增加Base_A 的初始化
    derivative(const string &p, const int &b, const int &c, const string &n) : phone(p), Bfor_A(b, n), Cfor_A(c, n), Base_A(n)
    {
